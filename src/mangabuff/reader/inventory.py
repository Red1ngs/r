...
if TYPE_CHECKING:
    ...
    from src.mangabuff.reader.models import ReaderWork, ItemReceivedEvent
...
class PersonalInventory(BaseInventory):
    ...
    def want_list(self) -> list[str]:
        return cast(list[str], self.data.get("want_list", []))
    ...
    def manga_have(self) -> list[str]:
        return cast(list[str], self.data.get("manga_have", []))
